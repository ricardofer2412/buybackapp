class BuyBacksController < ApplicationController
  ...
  private
    ...
    def buy_back_params
      params.require(:buy_back).permit(:remove_image, :first_name, :middle_name, :last_name, :address, :address_two, :city, :state, :zip_code, :email, :image, :drive_license,carriers_attributes:[:carrier], devices_attributes:[:id, :quantity, :device_name, :comments, :carrier, :imei, :price, :_destroy])
    end
end
